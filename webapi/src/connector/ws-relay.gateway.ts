...
export class WsRelayGateway implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    this.logger.log(
      'WS Relay Gateway initialized manually on port 8001 (ws://0.0.0.0:8001/ws/connectors)',
    );
  }
      ...
      if (internalService) {
        ...
        client.on('message', (data: Buffer) => {
          try {
            ...
            this.logger.error(
              `Failed to parse internal message: ${err.message}`,
            );
          }
        });
