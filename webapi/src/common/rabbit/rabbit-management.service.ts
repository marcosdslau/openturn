...
export class RabbitManagementService {
  ...
  async getOverview(): Promise<RabbitOverviewDto> {
    try {
      ...
      const queueTotalMessages = (q: {
        messages?: number;
        messages_ready?: number;
        messages_unacknowledged?: number;
      }) =>
        q.messages ??
        (q.messages_ready || 0) + (q.messages_unacknowledged || 0);
      ...
      this.logger.error(
        `Erro ao buscar overview do RabbitMQ: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getQueueDetail(queueName: string): Promise<RabbitQueueDetailDto> {
    ...
    try {
      ...
      this.logger.error(
        `Erro ao buscar fila RabbitMQ ${queueName}: ${error.message}`,
      );
      throw error;
    }
  }
