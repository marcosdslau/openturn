...
export function redisCronLockKey(rotinaId: number)      { return `${p()}:cron-lock:${rotinaId}:${Math.floor(Date.now() / 60000)}`; }

export function redisMonitorSnapshot()                  { return `${p()}:monitor:global:snapshot:v1`; }
export function redisMonitorRefreshLock()               { return `${p()}:monitor:global:snapshot:refresh:lock`; }
export function redisMonitorInstDashboard(instCodigo: number) { return `${p()}:monitor:instituicao:${instCodigo}:dashboard:v1`; }
export function redisMonitorInstDashboardPattern()      { return `${p()}:monitor:instituicao:*:dashboard:v1`; }
...
export function socketIoKeyPrefix()                     { return `${p()}:socket.io`; }
