...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const requiredRoles = this.reflector.getAllAndOverride<GrupoAcesso[]>(
      ROLES_KEY,
    ...
    // Global roles bypass
    const hasGlobalRole = user.acessos.some((a: any) =>
      GLOBAL_ROLES.includes(a.grupo),
    );
    if (hasGlobalRole) {
      return true;
    }
    ...
    const hasRole = user.acessos.some((a: any) =>
      requiredRoles.includes(a.grupo),
    );
    if (!hasRole) {
