...
export const ROUTINE_SCHEMA: SchemaTable[] = [
  {
    ...
    fields: [
      ...
      { name: 'PESFotoExtensao', type: 'String', description: 'Extensão Foto' },
      {
        name: 'PESGrupo',
        type: 'String',
        description: 'Grupo (Aluno, Prof...)',
      },
      {
        name: 'PESCartaoTag',
        type: 'String',
        description: 'Cartão/Tag Acesso',
      },
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        name: 'MATNumero',
        type: 'String',
        description: 'Número Matrícula (RA)',
      },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'REGTimestamp', type: 'BigInt', description: 'Unix Timestamp' },
      {
        name: 'REGDataHora',
        type: 'DateTime',
        description: 'Data/Hora legível',
      },
      {
        name: 'createdAt',
        type: 'DateTime',
        description: 'Data Criação Login',
      },
    ],
  },
  {
    ...
    fields: [
      {
        ...
        type: 'Int',
        pk: true,
        description: 'ID Configuração',
      },
      {
        name: 'ERPSistema',
        type: 'String',
        description: 'Nome do Sistema (Ex: Totvs)',
      },
      ...
      },
      {
        name: 'ERPConfigJson',
        type: 'Json',
        description: 'Parâmetros extras (JSON)',
      },
      {
        name: 'INSInstituicaoCodigo',
        type: 'Int',
        fk: 'INSInstituicao',
  ...
  },
  {
    ...
    fields: [
      ...
      {
        name: 'INSConfigHardware',
        type: 'Json',
        description: 'Configurações Globais Hardware',
      },
    ],
  },
  {
...
export const OPENAI_TOOLS: any[] = [
  {
    ...
    function: {
      ...
      description:
        'Retorna o schema completo do banco de dados disponível via context.db, incluindo nomes de tabelas/modelos, campos e seus tipos. Use quando precisar saber quais tabelas e colunas existem para montar queries Prisma.',
      parameters: {
        type: 'object',
        properties: {
          table_name: {
            type: 'string',
            description:
              'Nome opcional de uma tabela específica (ex: PESPessoa). Se vazio, retorna todas.',
          },
        },
        required: [],
      },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Retorna exemplos de código (snippets) prontos para uso no editor de rotinas. Categorias disponíveis: prisma, hardware, webhook, axios, logger, config, files. Use quando precisar de exemplos de como usar os helpers do sistema.',
      parameters: {
        type: 'object',
        properties: {
          category: {
            ...
            description:
              'Categoria de snippets desejada. Use "all" para retornar tudo.',
          },
        },
        required: ['category'],
      },
...
const SNIPPET_CATEGORIES: Record<string, { label: string; code: string }[]> = {
  prisma: [
    {
      label: 'Where (Operadores)',
      code: `context.db.PESPessoa.findMany({ where: { PESAtivo: true, PESNome: { contains: 'Silva', mode: 'insensitive' }, PESCodigo: { gt: 100 }, PESGrupo: { in: ['ALUNO','PROFESSOR'] } } })`,
    },
    {
      label: 'AND/OR',
      code: `context.db.PESPessoa.findMany({ where: { OR: [{ PESNome: { contains: 'João' } }], AND: [{ PESAtivo: true }] } })`,
    },
    {
      label: 'OrderBy',
      code: `context.db.MATMatricula.findMany({ orderBy: [{ MATCurso: 'asc' }, { createdAt: 'desc' }] })`,
    },
    {
      label: 'Paginação',
      code: `context.db.REGRegistroPassagem.findMany({ skip: 10, take: 10, orderBy: { REGTimestamp: 'desc' } })`,
    },
    {
      label: 'Include (Join)',
      code: `context.db.MATMatricula.findMany({ where: { MATAtivo: true }, include: { pessoa: true } })`,
    },
    {
      label: 'Select (campos)',
      code: `context.db.PESPessoa.findMany({ select: { PESNome: true, PESEmail: true } })`,
    },
    {
      label: 'GroupBy',
      code: `context.db.PESPessoa.groupBy({ by: ['PESGrupo'], _count: { PESCodigo: true } })`,
    },
  ],
  hardware: [
    {
    ...
    {
      label: 'deletePerson',
      code: `await context.hardware.deletePerson(pessoaId)`,
    },
    {
      label: 'setTag / removeTag',
      code: `await context.hardware.setTag(pessoaId, "tagCode"); await context.hardware.removeTag("tagCode")`,
    },
    {
      label: 'setFace / removeFace',
      code: `await context.hardware.setFace(pessoaId, "base64", "jpg"); await context.hardware.removeFace(pessoaId)`,
    },
    {
      label: 'setFingers / removeFingers',
      code: `await context.hardware.setFingers(pessoaId, ["t1","t2"]); await context.hardware.removeFingers(pessoaId)`,
    },
    {
    ...
    {
      label: 'executeAction',
      code: `await context.hardware.executeAction(equipId, "open_door", { door: 1 })`,
    },
    {
      label: 'enroll',
      code: `await context.hardware.enroll(equipId, "face", pessoaId)`,
    },
    {
      label: 'customCommand',
      code: `await context.hardware.customCommand(equipId, "load_objects", { object: "users", limit: 10 })`,
    },
  ],
  webhook: [
    ...
    {
      label: 'Headers',
      code: `const ct = context.request?.headers?.['content-type'];`,
    },
    {
      label: 'Method',
      code: `const method = context.request?.method; // GET, POST, etc.`,
    },
    {
      label: 'Path/Params',
      code: `const path = context.request?.path; const params = context.request?.params;`,
    },
  ],
  axios: [
    {
      label: 'GET simples',
      code: `const resp = await axios.get('https://api.example.com/data'); console.log(resp.data);`,
    },
    {
      label: 'Instância com Auth',
      code: `const api = axios.create({ baseURL: 'https://api.erp.com/v1', headers: { Authorization: 'Bearer TOKEN' } }); const { data } = await api.post('/sync', payload);`,
    },
  ],
  logger: [
    {
      label: 'log/info/error',
      code: `logger.log('msg'); logger.info('msg'); logger.error('msg');`,
    },
    {
      label: 'Try/Catch completo',
      code: `logger.info('Iniciando...'); try { /* ... */ logger.info('OK'); } catch(e) { logger.error(e.message); }`,
    },
  ],
  config: [
    {
      label: 'ERP Config',
      code: `const erp = await context.db.ERPConfiguracao.findFirst(); // erp.ERPUrlBase, erp.ERPToken`,
    },
    {
      label: 'Instituição',
      code: `const inst = await context.db.INSInstituicao.findFirst(); // inst.INSNome`,
    },
  ],
  files: [
    {
      label: 'Baixar imagem Base64',
      code: `const resp = await axios.get(url, { responseType: 'arraybuffer' }); const b64 = Buffer.from(resp.data,'binary').toString('base64');`,
    },
  ],
};
...
export function executeToolCall(
  ...
  switch (toolName) {
    ...
    case 'get_code_snippets': {
      ...
      if (!snippets)
        return JSON.stringify({
          error: `Categoria '${cat}' não encontrada. Use: ${Object.keys(SNIPPET_CATEGORIES).join(', ')}`,
        });
