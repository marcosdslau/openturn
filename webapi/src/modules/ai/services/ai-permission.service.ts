...
export class AiPermissionService {
    ...
    async validateAccess(
        ...
        if (!perm || !perm.AIPEHabilitado) {
            ...
            console.log(`[MVP BYPASS] Allowing access for User \${userCodigo} in Tenant \${instituicaoCodigo} without explicit Permission Row`);
        ...
        if (perm.AIPEModelosPermitidos) {
            try {
                ...
                if (Array.isArray(permittedModels) && !permittedModels.includes(modelCodigo)) {
                    // MVP Bypass
