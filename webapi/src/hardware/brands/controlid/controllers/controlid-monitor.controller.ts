...
import { ControlidPassagemService } from '../services/controlid-passagem.service';
import { ControlidSyncService } from '../services/controlid-sync.service';
...
export class ControlidMonitorController {
  ...
  constructor(
    private readonly passagemService: ControlidPassagemService,
  ...
  async getPush(
    ...
    this.logger.log(
      `[${codigoInstituicao}] [ControlID] Push query from device: ${deviceId}`,
    );
    return this.commandQueue.getPendingCommand(Number(deviceId));
  }
  ...
  async postResult(
    ...
    this.logger.log(
      `[${codigoInstituicao}] [ControlID] Result from device ${deviceId}`,
    );
    await this.commandQueue.processResult(Number(deviceId), body);
    return {};
  }
  ...
  async newUserIdentified(
    ...
    if (
      !(await this.deviceInstituicaoConfere(body?.device_id, codigoInstituicao))
    ) {
      this.logger.warn(
    ...
    return this.passagemService.validarAcessoOnline(body);
  }
  ...
  async syncPessoas(
    ...
    return this.syncService.syncPessoasToEquipamento(
      equipamentoCodigo,
      instituicaoCodigo,
    );
  }
  ...
  doorEvent(
    ...
    this.logger.log(
      `[${codigoInstituicao}] [ControlID] Door Event: device=${body?.device_id}`,
    );
    return {};
  }
  ...
  operationMode(
    ...
    this.logger.log(
      `[${codigoInstituicao}] [ControlID] Operation Mode Event received`,
    );
    return {};
  }
  ...
  template(
    ...
    this.logger.log(
      `[${codigoInstituicao}] [ControlID] Template Event received`,
    );
    return {};
  }
  ...
  faceTemplate(
    ...
    this.logger.log(
      `[${codigoInstituicao}] [ControlID] Face Template Event received`,
    );
    return {};
  }
  ...
  async catraEvent(
    ...
    await this.passagemService.registrarPassagem(body);
    return {};
  }
  ...
  async daoNotification(
    ...
    this.logger.log(
      `[${codigoInstituicao}] [ControlID] DAO Notification received`,
    );
    await this.monitorService.persistControlidDao(codigoInstituicao, body);
    await this.passagemService.tryRegisterPassagemAfterControlidDao(
      codigoInstituicao,
      body,
    );
    return {};
