...
export class RotinaService {
  ...
  async create(data: any, instituicaoCodigo: number, usuarioCodigo: number) {
    ...
    if (
      data.ROTTipo === 'WEBHOOK' &&
      data.ROTWebhookPath &&
      data.ROTWebhookMetodo
    ) {
    ...
    if (
      rotina.ROTTipo === TipoRotina.SCHEDULE &&
      rotina.ROTAtivo &&
  ...
  private resolveRunningExecutionRow(row: { EXEIdExterno: string }): {
    running: boolean;
    exeId: string | null;
  } {
  ...
  async clearSerialExecutionLock(
    rotinaCodigo: number,
    instituicaoCodigo: number,
  ) {
    const existing = await this.prisma.rOTRotina.findFirst({
    ...
    return this.rotinaQueueService.clearSerialInflightZset(
      instituicaoCodigo,
      rotinaCodigo,
    );
  }
        ...
        .map((exec) => {
          ...
          if (Array.isArray(filteredLogs)) {
            filteredLogs = filteredLogs.filter((log) => {
              ...
              if (searchLower) {
                const msgMatch = log.message
                  ?.toLowerCase()
                  .includes(searchLower);
                const levelMatch = log.level
                  ?.toLowerCase()
                  .includes(searchLower);
          ...
          const executionErrorMatch =
            searchLower && exec.EXEErro?.toLowerCase().includes(searchLower);
