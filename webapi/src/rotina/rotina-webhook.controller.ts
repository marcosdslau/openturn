...
export class RotinaWebhookController {
  ...
  async handleWebhook(
    ...
    if (!rotina.ROTAtivo) {
      this.logger.debug(
        `Webhook recebido para rotina inativa ${rotina.ROTCodigo}. Ignorando.`,
      );
    ...
    if (rotina.ROTWebhookSeguro && rotina.ROTWebhookToken) {
      ...
      if (receivedToken !== rotina.ROTWebhookToken) {
        this.logger.warn(
          `Tentativa de acesso não autorizado ao webhook ${path}. Fonte: ${tokenSource}, Chave: ${tokenKey}`,
        );
    ...
    if (rotina.ROTWebhookAguardar) {
      try {
        const execSec = routineTimeoutSecondsFromCadastro(
    ...
    return res
      .status(202)
      .json({ message: 'Webhook received', exeId, execution: 'queued' });
  }

  private async waitForResultViaRedis(
    exeId: string,
    timeoutMs: number,
  ): Promise<any> {
