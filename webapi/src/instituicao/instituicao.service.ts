...
export class InstituicaoService {
  ...
  async bulkUpdateWorkerStatus(active: boolean) {
    await this.prisma.iNSInstituicao.updateMany({
      data: { INSWorkerAtivo: active },
    });
  ...
  private async publishQueueRefresh(
    ...
    try {
      ...
      this.logger.warn(
        `Falha ao publicar refresh de instituição ${instituicao.INSCodigo}: ${(error as Error).message}`,
      );
    }
  }
  ...
  private async publishReconcileAll() {
    ...
    try {
      ...
      this.logger.warn(
        `Falha ao publicar reconcileAll: ${(error as Error).message}`,
      );
    }
  }
}
