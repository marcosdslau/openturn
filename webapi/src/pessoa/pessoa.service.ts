...
export class PessoaService {
  ...
  async findAll(
    ...
    const { page, limit, nome, documento, email, grupo, cartaoTag, ativo, foto, erro } =
      query;
    ...
    const where: Prisma.PESPessoaWhereInput = {
      ...
      ...(documento && {
        PESDocumento: { contains: documento, mode: 'insensitive' },
      }),
  ...
  async findOne(instituicaoCodigo: number, id: number) {
    ...
    if (!pessoa)
      throw new NotFoundException(
        `Pessoa ${id} não encontrada para esta instituição`,
      );
    return pessoa;
  }
