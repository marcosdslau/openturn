...
import {
    ...
    EntrySide,
} from '../hardware/interfaces/hardware.types';
...
export function extractControlidCatraRotationCode(body: any): number | null {
    ...
    if (typeof top === 'string' && /^-?\d+$/.test(String(top).trim())) return Number(String(top).trim());
    return null;
}
...
function acaoFromEquipmentSide(side: EntrySide, rotation: number | null): AcaoPassagem | null {
    if (rotation !== 7 && rotation !== 8) return null;
    const clockwise = rotation === 7;
    const entradaIsClockwise = side === 'right';
    return clockwise === entradaIsClockwise ? AcaoPassagem.ENTRADA : AcaoPassagem.SAIDA;
}

/** Normaliza host/IP para comparação (sem porta, sem protocolo, minúsculas). */
export function normalizeControlidHostOrIp(s: string | undefined | null): string | null {
...
export function isPrimaryControlidCatraDevice(
    ...
    if (entryId && deviceIdStr === entryId && ipEntry && ipEntry === eqpIp) return true;

    const ipExit = normalizeControlidHostOrIp(config.ip_exit);
    if (exitId && deviceIdStr === exitId && ipExit && ipExit === eqpIp) return true;
...
export function tryDualReaderAcaoPassagem(
    ...
    if (entry && matchedField === 'EQPConfig.deviceId_entry' && deviceIdStr === entry) {
        return AcaoPassagem.ENTRADA;
    }
    if (exit && matchedField === 'EQPConfig.deviceId_exit' && deviceIdStr === exit) {
        return AcaoPassagem.SAIDA;
    }
    return undefined;
}
...
export function resolveControlidCatraAcaoPassagem(params: {
    ...
    const deviceIdStr = params.body?.device_id != null ? String(params.body.device_id) : '';
