...
export class MonitorInstituicaoDashboardService {
  ...
  async getDashboard(
  ...
  ): Promise<MonitorInstituicaoDashboardDto> {
    ...
    const inst = snapshot.instituicoes.find(
      (i) => i.codigo === instituicaoCodigo,
    );
    if (!inst) {
    ...
    const extras =
      await this.monitorSnapshotService.getInstituicaoDashboardExtrasCached(
        instituicaoCodigo,
        () =>
          this.builder.buildInstituicaoDashboardExtras(
            instituicaoCodigo,
            now,
            tz,
          ),
    ...
    const [waiting, active, rabbit] = await Promise.all([
      ...
      this.rabbitMgmtService.getQueueDetail(
        getMainQueueName(instituicaoCodigo),
      ),
    ]);
