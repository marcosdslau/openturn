...
export class MonitorSnapshotBuilder {
  ...
  async build(params: {
    ...
    Omit<MonitorSnapshotDto, 'version' | 'generatedAt' | 'refreshDurationMs'>
  > {
    ...
    const execMap = new Map(
      execShortRows.map((r) => [r.INSInstituicaoCodigo, r]),
    );
    const durMap = new Map(
      durationRows.map((r) => [r.INSInstituicaoCodigo, r]),
    );
    const pessoaMap = new Map(
      pessoasByInst.map((r) => [r.INSInstituicaoCodigo, r._count]),
    );
    const matMap = new Map(
      matriculasByInst.map((r) => [r.INSInstituicaoCodigo, r._count]),
    );

    const statusByInstWindow = new Map<
      JanelaStatus,
      Map<number, StatusContagem>
    >();
    ...
    const instituicoesOut: InstituicaoMonitorSnapshot[] = instituicoesBase.map(
      (ib) => {
        ...
        for (const jw of JANELAS_CURTAS) {
          topRotinasPorJanelaCurta[jw] =
            topShortMap.get(jw)?.get(ib.INSCodigo) ?? [];
        }

        const statusPorJanela = {} as Record<JanelaStatus, StatusContagem>;
        for (const j of JANELAS_STATUS) {
          statusPorJanela[j] =
            statusByInstWindow.get(j)?.get(ib.INSCodigo) ??
            emptyStatusContagem();
        }
  ...
  private async buildSeries(params: {
    ...
    const { tzEsc, cut10h, cut24h, cut5d, cut15d, cut30d, instituicaoCodigo } =
      params;

    const hourQuery = async (
      since: Date,
    ): Promise<{ bucket: string; cnt: bigint }[]> => {
