...
### Frequência / registros diários (Gennera)

Além das rotinas JavaScript, existe fluxo dedicado para **agregar passagens** (`REGRegistroPassagem` com `REGProcessado = false`) em **`RPDRegistrosDiarios`** e opcionalmente enviar ao Gennera:

| Etapa | Onde roda | Descrição |
|-------|-----------|-----------|
| Agendamento | `webapi` (`RegistroDiarioSyncScheduler`) | Lê `INSTempoSync` e `INSSyncRegistrosDiarios` por instituição; no tick, se houver passagens pendentes, publica job RabbitMQ com `trigger: INTERNAL`. |
| Agregação | `worker` | Consome job INTERNAL, agrupa por pessoa e dia (fuso `INSFusoHorario`), faz upsert em `RPDRegistrosDiarios`, marca passagens como processadas. |
| Lançamento manual | `webapi` (`GenneraAttendanceService` + Redis para progresso) | UI "Administrar" chama `POST /instituicao/:id/registro-diario/gennera/lancamento` e faz polling do percentual; apenas ERP `Gennera` habilitado neste fluxo. |

Endpoints Gennera usados: `POST /persons/{PESIdExterno}/attendances/interval` e `POST /persons/{PESIdExterno}/attendances`, com `ERPUrlBase` e `ERPToken` de `ERPConfiguracao`.
