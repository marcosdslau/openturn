...
const AppHeader: React.FC = () => {
  ...
  const searchableItems = useMemo(() => {
    ...
    const processItems = (items: any[], sectionPrefix?: string) => {
      items.forEach((item) => {
        if (item.subItems) {
          processItems(item.subItems, item.name);
        } else if (item.path) {
          flattened.push({
            name: item.name,
            path: item.path,
            section: sectionPrefix || item.section, // Use provided section or undefined
          });
        }
      });
    };

    processItems(mainNavItems);
    processItems(demoNavItems, "TailAdmin");
    processItems(supportItems, "Support");
    processItems(othersItems, "Others");
    ...
    <header className="sticky top-0 flex w-full bg-white border-gray-200 z-40 dark:border-gray-800 dark:bg-gray-900 xl:border-b">
      <div className="flex flex-col items-center justify-between grow xl:flex-row xl:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 xl:justify-normal xl:border-b-0 xl:px-0 xl:py-4">
          <button
            className={`flex items-center justify-center w-10 h-10 text-gray-500 border-gray-200 rounded-lg z-40 dark:border-gray-800 dark:text-gray-400 lg:h-11 lg:w-11 xl:border lg:bg-transparent dark:lg:bg-transparent ${isMobileOpen ? "bg-gray-100 dark:bg-white/[0.03]" : ""
              }`}
          ...
          <button
            ...
            className="flex items-center justify-center w-10 h-10 text-gray-700 rounded-lg z-40 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 xl:hidden"
          >
            <svg
              width="24"
              height="24"
            ...
            <form onSubmit={(e) => e.preventDefault()}>
              <div className="relative">
                ...
                <input
                  ref={inputRef}
                  type="text"
                  placeholder="Buscar no menu ou tecle ⌘+K..."
                  value={searchQuery}
                ...
                {searchQuery && (
                  <div className="absolute top-full left-0 w-full mt-2 bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700 overflow-hidden z-50">
                        ...
                        searchResults.map((item, index) => (
                          <li key={`${item.name}-${index}`}>
                            <Link
                              ...
                              className="block px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
                            >
